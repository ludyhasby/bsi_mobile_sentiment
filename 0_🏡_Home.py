...
with tab1:
    ...
    with kol1:
        ...
        with col2:
            play_store_count = len(df[df['sumber'] == 'ps'])
            apps_store_count = len(df[df['sumber'] == 'as'])
...
with tab3:
    ...
    input_pilih = st.selectbox("Input yang diinginkan", options=["Teks", "Upload File"], index=0)
    if input_pilih == "Teks":
        teks = st.text_input("Masukkan Kalimat disini")
        if (teks):
            teks_prep = [preprocessing(teks)]
            teks_padded = teks_to_pad(teks_prep)
            result_ = model.predict(teks_padded)
            if result_ >= 0.5:
                st.markdown(f'<span style="font-size: 50px;">:green[Positive]</span>', unsafe_allow_html=True)
                st.write(f"Dengan Keyakinan: {(result_[0][0])*100:.2f}%")
            else:
                st.markdown(f'<span style="font-size: 50px;">:red[Negative]</span>', unsafe_allow_html=True)
                st.write(f"Dengan Keyakinan: {(1-result_[0][0])*100:.2f}%")
    elif input_pilih == "Upload File":
        file_baru = st.file_uploader("Ambil File CSV", type=["csv"], accept_multiple_files=False)
        if file_baru:
            dtest = pd.read_csv(file_baru)
            st.write(dtest.head())
            kolomSentences = st.selectbox("Yang mana Kolom Sentences yang ingin diprediksi ?", options=dtest.columns)
            if st.button("Process"):
                st.write("Preprocessing.....")
                stqdm.pandas()
                dtest["Preprocess_Sentences"] = dtest[kolomSentences].progress_apply(preprocessing)
                st.write("Generate Not Null....")
                dtest = dtest[dtest["Preprocess_Sentences"].notna()]
                preprocess_sentences = dtest["Preprocess_Sentences"].tolist()
                st.write("Padding..")
                padding_sentences = teks_to_pad(preprocess_sentences)
                st.write("Model Predict..")
                stqdm.pandas()
                result = model.predict(padding_sentences)
                dtest["prob_keyakinan"] = result
                dtest["sentiment"] = ["positive" if i >= 0.5 else "negative" for i in result]
                st.write(dtest[[kolomSentences, "prob_keyakinan", "sentiment"]].head())
                
                sentiment = dtest.groupby(["sentiment"]).size().reset_index()
                sentiment.columns = ["sentiment", "size"]
                fig = px.pie(sentiment, values='size', names='sentiment', 
                                    title=f'Distribusi Sentiment Data User', color_discrete_sequence= ['#3EA5A1', '#ff0000'])
                st.plotly_chart(fig, theme="streamlit")

                data = dtest.to_csv(index=False)
                st.download_button(
                    label="Download Data Hasil Pemodelan",
                    data=data,
                    file_name="df_result_sentiment.csv",
                    mime="text/csv",
                )
...
with tab4:
    with st.container():
        ...
        with image_column:
            ...
            st.markdown(":green[Aspiring Machine Learning Engineer]")
